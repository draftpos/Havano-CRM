...
def get_context():
	from havano_crm.api import check_app_permission

	if not check_app_permission():
		frappe.throw(_("You do not have permission to access Havano CRM"), frappe.PermissionError)

	frappe.db.commit()
	context = frappe._dict()
	context.boot = get_boot()
	if frappe.session.user != "Guest":
		capture("active_site", "havano_crm")
	return context
	...
	return frappe._dict(
		{
			...
			"demo_data_created": frappe.db.get_default("havano_crm_demo_data_created") == "1",
