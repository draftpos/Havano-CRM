...
from havano_crm.api.views import get_views
from havano_crm.compat import get_form_script
from havano_crm.utils import get_dynamic_linked_docs, get_linked_docs, is_frappe_version


def _resolved_list_payload(controller, doctype: str):
	if hasattr(controller, "default_list_data"):
		return controller.default_list_data()
	return get_standard_list_data(doctype)


def _resolved_kanban_settings(controller, doctype: str):
	if hasattr(controller, "default_kanban_settings"):
		return controller.default_kanban_settings()
	return get_standard_kanban_settings(doctype)


def _normalize_erpnext_lead_filters(filters: frappe._dict):
	"""CRM Lead used a `converted` check field; ERPNext Lead uses status 'Converted'."""
	if "converted" not in filters:
		return
	val = filters.pop("converted")
...
class _ListFieldDoctypeContext:
	__slots__ = ("doctype",)
...
def _list_field_allowed(doctype: str, fieldname: str) -> bool:
	"""True if fieldname is safe for frappe.get_list on this doctype (matches client.get_list validation)."""
	if not fieldname:
		return False
	raw = str(fieldname).strip()
	if not raw:
		return False
	# expressions / aggregates — not used in CRM list rows, but do not strip blindly
	if "(" in raw or " AS " in raw.upper():
		return True
	base = raw.split(".")[-1].strip("`") if "." in raw else raw.strip("`")
	if is_standard(base):
		return True
	_, df = get_meta_and_docfield(raw, _ListFieldDoctypeContext(doctype))
	return bool(df)


def _sanitize_list_rows_and_columns(doctype: str, rows: list, columns: list | None):
	"""Drop fields that are not on the DocType (e.g. legacy Frappe CRM `enabled` on Lead views)."""
	rows = rows or []
	...
	for r in rows:
		if not r or r in seen:
			continue
		if _list_field_allowed(doctype, r):
			clean_rows.append(r)
			seen.add(r)
	clean_columns = [c for c in columns if c.get("key") and _list_field_allowed(doctype, c["key"])]
	if not clean_rows:
		clean_rows = ["name"]
	elif "name" not in clean_rows:
		clean_rows.insert(0, "name")
...
def get_data(
	...
	if doctype == "Lead":
		_normalize_erpnext_lead_filters(filters)
	...
	if group_by_field and not _list_field_allowed(doctype, group_by_field):
		group_by_field = None
	...
	kanban_settings = _resolved_kanban_settings(_list, doctype)
	...
	if view_type != "kanban":
		...
		if (
			not custom_view
			and frappe.db.exists("DocType", "CRM View Settings")
			and frappe.db.exists("CRM View Settings", default_view_filters)
		):
			list_view_settings = frappe.get_doc("CRM View Settings", default_view_filters)
			columns = frappe.parse_json(list_view_settings.columns)
			rows = frappe.parse_json(list_view_settings.rows)
			is_default = False
		elif not custom_view or (is_default and list_payload):
			rows = default_rows
			columns = list_payload.get("columns") or []
		...
		if (
			group_by_field
			and _list_field_allowed(doctype, group_by_field)
			and group_by_field not in rows
		):
	...
	if view_type == "kanban":
		...
		if not column_field and kanban_settings:
			column_field = kanban_settings.get("column_field")
		...
		if not title_field:
			title_field = "name"
			if kanban_settings:
				title_field = kanban_settings.get("title_field") or title_field
		...
		if not kanban_fields:
			kanban_fields = ["name"]
			if kanban_settings and kanban_settings.get("kanban_fields"):
				kanban_fields = json.loads(kanban_settings.get("kanban_fields"))

		for field in kanban_fields:
			if field not in rows:
				rows.append(field)

		rows, _unused_columns = _sanitize_list_rows_and_columns(doctype, rows, [])
		kanban_fields = [f for f in kanban_fields if _list_field_allowed(doctype, f)]
		if not kanban_fields:
			kanban_fields = ["name"]
	...
	if not is_default and custom_view_name and frappe.db.exists("DocType", "CRM View Settings"):
...
def parse_list_data(data, doctype):
	...
	if doctype == "Lead" and isinstance(data, list):
		_enrich_reference_todo_summaries(data, "Lead")
	elif doctype == "Opportunity" and isinstance(data, list):
		_enrich_reference_todo_summaries(data, "Opportunity")
	return data
...
def getCounts(d, doctype):
	...
	d["_task_count"] = frappe.db.count(
		"ToDo",
		filters={"reference_type": doctype, "reference_name": d.get("name")},
	)
	d["_note_count"] = frappe.db.count(
		"Note", filters={"reference_doctype": doctype, "reference_docname": d.get("name")}
	)
	return d
