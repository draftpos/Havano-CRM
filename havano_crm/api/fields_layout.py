...
# SPDX-License-Identifier: MIT. Adapted for Havano CRM without requiring the crm app.
...
def _crm_fields_layout_available() -> bool:
	return bool(frappe.db.exists("DocType", "CRM Fields Layout"))
...
def get_fields_layout(doctype: str, type: str, parent_doctype: str | None = None):
	...
	if _crm_fields_layout_available() and frappe.db.exists("CRM Fields Layout", {"dt": doctype, "type": type}):
...
def get_sidepanel_sections(doctype: str):
	if not _crm_fields_layout_available() or not frappe.db.exists(
...
def save_fields_layout(doctype: str, type: str, layout: str):
	if not _crm_fields_layout_available():
		frappe.throw(
			_(
				"Custom field layouts need DocType CRM Fields Layout (Frappe CRM app). "
				"Use Desk → Customize Form for standard forms."
			)
		)
