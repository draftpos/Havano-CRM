...
def _should_update_modified(doc: Communication | Comment) -> bool:
	...
	if doc.reference_doctype not in ["Lead", "Opportunity"]:
		return False
	...
	if frappe.db.exists("DocType", "FCRM Settings"):
		try:
			return bool(
				frappe.db.get_single_value("FCRM Settings", "update_timestamp_on_new_communication")
			)
		except Exception:
			return False
...
def create_lead_from_incoming_email(doc: Communication, method: str | None = None):
	...
	if frappe.db.exists("Lead", {"email_id": doc.sender}):
		return

	lead = frappe.new_doc("Lead")
	lead.email_id = doc.sender
...
def on_comment_insert(doc: Comment, method: str | None = None):
	...
	if doc.reference_doctype not in ["Lead", "Opportunity"] or doc.comment_type != "Comment":
		return
