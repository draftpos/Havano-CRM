...
class Twilio:
	...
	def get_recording_status_callback_url(self):
		url_path = "/api/method/havano_crm.integrations.twilio.api.update_recording_info"
		return get_public_url(url_path)

	def get_update_call_status_callback_url(self):
		url_path = "/api/method/havano_crm.integrations.twilio.api.update_call_status_info"
		return get_public_url(url_path)
	...
	if len(current_loggedin_users) > 1 and caller:
		...
		if not opportunity_owner:
			opportunity_owner = frappe.db.get_value(
				"Lead",
				{"mobile_no": caller, "status": ["!=", "Converted"]},
				"lead_owner",
			)
