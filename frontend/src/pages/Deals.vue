<template>
  ...
  <DealsListView
    ...
    "
    @open-activities="openDealActivitiesModal"
  />
  <EmptyState
    v-else-if="deals.data && !rows.length"
    name="Deals"
  ...
  />
  <LeadOpenTodosModal
    v-model="showDealTodosModal"
    reference-doctype="Opportunity"
    :lead-name="todosModalDeal"
    @edit="onEditTodoFromDealModal"
    @updated="onDealTodosModalUpdated"
  />
</template>
...
import LeadOpenTodosModal from '@/components/Modals/LeadOpenTodosModal.vue'
...
function effectiveRowKeys() {
  const base = deals.value?.data?.rows || []
  const vt = deals.value?.data?.view_type
  if (
    (vt === 'list' || vt === 'group_by') &&
...
const columns = computed(() => {
  ...
  if (
    (vt === 'list' || vt === 'group_by') &&
    !_columns.some((c) => c.key === '_activities_todo')
  ) {
    const actCol = {
      key: '_activities_todo',
      label: __('Activities'),
      type: 'Data',
      width: '11rem',
    }
...
function getGroupedByRows(listRows, groupByField, columns) {
  ...
  groupByField.options?.forEach((option) => {
    ...
    let groupDetail = {
      ...
      group: option || __(' '),
      collapsed: false,
      rows: parseRows(filteredRows, columns, effectiveRowKeys()),
    }
...
function parseRows(rows, columns = [], rowKeys = null) {
  let view_type = deals.value.data.view_type
  let key = view_type === 'kanban' ? 'fieldname' : 'key'
  let type = view_type === 'kanban' ? 'fieldtype' : 'type'

  const keys = rowKeys ?? deals.value?.data?.rows ?? []
  ...
  return rows.map((deal) => {
    ...
    keys.forEach((row) => {
      ...
      if (row == 'organization') {
        ...
        _rows[row] = {
          label: deal.opportunity_owner && getUser(deal.opportunity_owner).full_name,
          ...(deal.opportunity_owner && getUser(deal.opportunity_owner)),
        }
      } else if (row == '_assign') {
...
const showDealTodosModal = ref(false)
const todosModalDeal = ref('')

function openDealActivitiesModal(dealName) {
  todosModalDeal.value = dealName
  showDealTodosModal.value = true
}

function onEditTodoFromDealModal(t) {
  showDealTodosModal.value = false
  docname.value = todosModalDeal.value
  task.value = {
    name: t.name,
    title: '',
    ...
    reference_docname: todosModalDeal.value,
  }
  showTaskModal.value = true
}

function onDealTodosModalUpdated() {
