<template>
  ...
  <div v-if="doc.name" class="flex h-full overflow-hidden bg-[#eef1f6]">
    <Tabs
      v-model="tabIndex"
      as="div"
      :tabs="tabs"
      class="nexus-record-tabs flex min-w-0 flex-1 flex-col overflow-hidden bg-white shadow-sm [&_.absolute.rounded-full>div]:!bg-emerald-500 [&_[data-state=active]]:!text-emerald-800 [&_[role='tab']]:shrink-0 [&_[role='tab']]:px-0 [&_[role='tablist']]:min-h-[48px] [&_[role='tablist']]:gap-8 [&_[role='tablist']]:border-b [&_[role='tablist']]:border-slate-200/80 [&_[role='tablist']]:px-5 [&_[role='tabpanel']:not([hidden])]:flex [&_[role='tabpanel']:not([hidden])]:grow"
    >
      <template #tab-panel>
        <Activities
          ref="activities"
    ...
    <Resizer
      side="right"
      class="flex flex-col justify-between border-l border-slate-200/90 bg-white shadow-[-12px_0_32px_-20px_rgba(15,23,42,0.18)]"
    >
      <div
      ...
      <LeadSideActivityPreview
        ref="sidePreview"
        doctype="Opportunity"
        :docname="dealId"
        :task-actions="sideTaskActions"
        @view-all="changeTabTo('activity')"
        @task-updated="onSideTaskUpdated"
      />
      <SLASection
        v-if="doc.sla_status"
        v-model="doc"
...
import LeadSideActivityPreview from '@/components/Activities/LeadSideActivityPreview.vue'
...
const { tabIndex, changeTabTo } = useActiveTabManager(tabs, 'lastDealTab', 'todo')

const sections = createResource({
  url: 'havano_crm.api.fields_layout.get_sidepanel_sections',
...
async function addContact(contact) {
  ...
  let d = await call('havano_crm.api.opportunity.add_contact', {
    opportunity: props.dealId,
    contact,
  })
  if (d) {
    dealContacts.reload()
...
async function removeContact(contact) {
  let d = await call('havano_crm.api.opportunity.remove_contact', {
    opportunity: props.dealId,
    contact,
  })
  if (d) {
...
async function setPrimaryContact(contact) {
  let d = await call('havano_crm.api.opportunity.set_primary_contact', {
    opportunity: props.dealId,
    contact,
  })
  if (d) {
...
const dealContacts = createResource({
  url: 'havano_crm.api.opportunity.get_opportunity_contacts',
...
const sidePreview = ref(null)

const sideTaskActions = computed(() => ({
  edit: (t) => activities.value?.openTaskModal?.(t),
  markDone: (t) => activities.value?.markTaskDone?.(t),
  cancel: (t) => activities.value?.cancelTask?.(t),
}))

function onSideTaskUpdated() {
  activities.value?.all_activities?.reload()
  sidePreview.value?.reloadFeed?.()
}
...
function statusLabel(status) {
  if (isTranslatable('Opportunity')) return __(status)
  return status
}
...
function reloadResources(data) {
  if (Object.hasOwn(data ?? {}, 'opportunity_owner')) {
    assignees.reload()
  }
  if (
    Object.hasOwn(data ?? {}, 'status') &&
