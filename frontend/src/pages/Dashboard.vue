...
const dashboardItems = createResource({
  url: 'havano_crm.api.dashboard.get_dashboard',
  makeParams() {
    return {
...
const saveDashboard = createResource({
  url: 'havano_crm.api.dashboard.save_dashboard_layout',
  method: 'POST',
  onSuccess: () => {
    dashboardItems.reload()
    editing.value = false
...
function resetToDefault() {
  createResource({
    url: 'havano_crm.api.dashboard.reset_to_default',
    auto: true,
    onSuccess: () => {
      dashboardItems.reload()
      editing.value = false
...
usePageMeta(() => {
  return { title: __('Havano CRM Dashboard') }
})
</script>
