<template>
  ...
  <LeadsListView
    ...
    "
    @open-activities="openLeadActivitiesModal"
  />
  <EmptyState
    v-else-if="leads.data && !rows.length"
    name="Leads"
  ...
  />
  <LeadOpenTodosModal
    v-model="showLeadTodosModal"
    reference-doctype="Lead"
    :lead-name="todosModalLead"
    @edit="onEditTodoFromModal"
    @updated="onLeadTodosModalUpdated"
  />
</template>
...
function getKanbanRows(data, columns) {
  ...
  return parseRows(_rows, columns, leads.value?.data?.rows || [])
}
...
function parseRows(rows, columns = [], rowKeys = null) {
  ...
  const keys = rowKeys ?? leads.value?.data?.rows ?? []
...
const showLeadTodosModal = ref(false)
const todosModalLead = ref('')

function openLeadActivitiesModal(leadName) {
  todosModalLead.value = leadName
  showLeadTodosModal.value = true
}

function onEditTodoFromModal(t) {
  showLeadTodosModal.value = false
  docname.value = todosModalLead.value
  task.value = {
    name: t.name,
    title: '',
    ...
    reference_docname: todosModalLead.value,
  }
  showTaskModal.value = true
}

function onLeadTodosModalUpdated() {
