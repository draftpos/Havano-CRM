<template>
  ...
  <ViewControls
    ...
    doctype="ToDo"
    :filters="todoListDefaultFilters"
    :options="{
...
/** List view only: open todos + due-date sort (sort set in ViewControls). */
const todoListDefaultFilters = computed(() =>
  route.params.viewType === 'list' ? { status: 'Open' } : {},
)
...
function parseRows(rows, columns = []) {
  ...
  return rows.map((task) => {
    ...
    tasks.value?.data.rows.forEach((row) => {
      ...
      } else if (row == 'assigned_to' || row == 'allocated_to') {
        const uid = task.allocated_to || task.assigned_to
        _rows[row] = {
          label: uid && getUser(uid).full_name,
          ...(uid && getUser(uid)),
        }
      }
    })
    _rows.name = task.name
    _rows._rawDate = task.date
    _rows.description = task.description
    _rows.status = task.status
    _rows.priority = task.priority
    _rows.reference_type = task.reference_type
    _rows.reference_name = task.reference_name
    _rows.allocated_to = task.allocated_to
    return _rows
  })
}
...
function showTask(name) {
  ...
  task.value = {
    ...
    assigned_to: t.allocated_to || t.assigned_to?.name || '',
    due_date: t._rawDate || t.date,
    status: t.status,
    priority: t.priority,
    reference_doctype: t.reference_type || t.reference_doctype,
    reference_docname: t.reference_name || t.reference_docname,
  }
...
function redirect(doctype, docname) {
  if (!docname) return
  let name = doctype == 'Opportunity' ? 'Deal' : 'Lead'
