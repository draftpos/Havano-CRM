...
/** Legacy URL/storage used #tasks; tab is now named Todo. */
function normalizeStoredOrHashTab(tabName) {
  if (!tabName) return tabName
  const n = String(tabName).toLowerCase()
  if (n === 'tasks') return 'todo'
  return n
}

export function useActiveTabManager(tabs, storageKey, defaultTabName = 'activity') {
  const activeTab = useStorage(storageKey, defaultTabName)
  ...
  function findTabIndex(tabName) {
    const key = normalizeStoredOrHashTab(tabName) || tabName
    return tabs.value?.findIndex(
      (tabOptions) => tabOptions.name.toLowerCase() === key,
    )
  }
  ...
  function getActiveTab() {
    let _activeTab = normalizeStoredOrHashTab(getActiveTabFromUrl())
    if (_activeTab) {
    ...
    let lastVisitedTab = normalizeStoredOrHashTab(activeTab.value)
    if (lastVisitedTab) {
  ...
  watch(
    ...
    (tabValue) => {
      ...
      let tabName = normalizeStoredOrHashTab(tabValue.replace('#', ''))
      let index = findTabIndex(tabName)
      if (index === -1) index = 0
