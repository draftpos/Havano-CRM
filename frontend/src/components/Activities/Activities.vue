<template>
  ...
  <FadedScrollableDiv class="flex flex-col h-full overflow-y-auto">
    ...
    >
      ...
      <div v-else-if="title == 'Todo'" class="px-3 pb-3 sm:px-10 sm:pb-5">
...
const whatsappMessages = createResource({
  url: 'havano_crm.api.whatsapp.get_whatsapp_messages',
...
function sendTemplate(template) {
  ...
  createResource({
    url: 'havano_crm.api.whatsapp.send_whatsapp_template',
    params: {
      reference_doctype: props.doctype,
      reference_name: props.docname,
...
function scroll(hash) {
  if (['todo', 'notes', 'events'].includes(route.hash?.slice(1))) return
  setTimeout(() => {
    let el
    if (!hash) {
...
function openTaskModal(t) {
  modalRef.value?.showTask(t)
}

function markTaskDone(t) {
  modalRef.value?.updateTaskStatus?.('Done', t)
}

function cancelTask(t) {
  modalRef.value?.updateTaskStatus?.('Canceled', t)
}
