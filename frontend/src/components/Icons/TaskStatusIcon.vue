<template>
  ...
  >
    ...
    />
    <path
      v-else-if="normalized == 'In Progress'"
      fill="currentColor"
      fill-rule="evenodd"
    ...
    />
    <circle v-else-if="normalized" cx="8" cy="8" r="7" fill="currentColor" />
  </svg>
</template>
<script setup>
...
/** ERPNext ToDo: Open | Closed | Cancelled — map to CRM icon shapes */
...
const statusColorClass = computed(() => {
  const s = props.status
  if (['Open', 'Todo', 'Backlog'].includes(s)) return 'text-sky-600'
  if (s === 'In Progress') return 'text-amber-600'
  if (['Done', 'Closed'].includes(s)) return 'text-emerald-600'
  if (['Canceled', 'Cancelled'].includes(s)) return 'text-slate-500'
  return s ? 'text-ink-gray-7' : 'text-ink-gray-5'
})
</script>
