...
async function convertToDeal() {
  ...
  let _deal = await call('havano_crm.api.opportunity.convert_lead_to_deal', {
    lead: props.lead.name,
  ...
  const dealId = typeof _deal === 'string' ? _deal : _deal?.message
  if (dealId) {
    show.value = false
    ...
    updateOnboardingStep('convert_lead_to_deal', true, false, () => {
      localStorage.setItem('firstDeal' + user, dealId)
    })
